...
impl AudioVisualizer {
    ...
    pub fn get_bars_and_peaks(
        ...
        let is_active = is_playing && now.duration_since(state.last_active) < Duration::from_millis(350);
        ...
        let dt = now.duration_since(state.last_frame_time).as_secs_f32().clamp(0.005, 0.1);
        ...
        for i in 0..bar_count {
            ...
            let next_peak = if next_peak < 0.01 { 0.0 } else { next_peak.min(1.0) };
...
fn compute_fft_magnitudes(samples: &[f32; FFT_SIZE], twiddles: &[(f32, f32)]) -> [f32; FFT_SIZE / 2] {
    let mut re = *samples;
    let mut im = [0.0f32; FFT_SIZE];

    // Bit reversal permutation
    for i in 0..FFT_SIZE {
        let rev = (i.reverse_bits() >> (usize::BITS - 10)) as usize;
        if i < rev {
            re.swap(i, rev);
            im.swap(i, rev);
        }
...
fn calculate_frequency_bands(magnitudes: &[f32; FFT_SIZE / 2], bar_count: usize) -> Vec<f32> {
    ...
    for i in 0..bar_count {
        ...
        let right = if i + 1 < bar_count { raw_bands[i + 1] } else { raw_bands[i] };
...
mod tests {
    ...
    fn test_frequency_bands_bass_vs_treble() {
        ...
        let bass_peak_bar = bass_bars
            ...
            .unwrap()
            .0;
        assert!(bass_peak_bar <= 3, "60 Hz should peak in low bass bars (<= 3), got {bass_peak_bar}");
        ...
        let treble_peak_bar = treble_bars
            ...
            .unwrap()
            .0;
        assert!(treble_peak_bar >= 11, "8 kHz should peak in high bars (>= 11), got {treble_peak_bar}");
    }
    ...
    fn test_audio_visualizer_push_and_decay() {
        ...
        assert!(bars.iter().any(|&b| b > 0), "Active audio should yield non-zero bars");
        assert!(peaks.iter().any(|&p| p > 0), "Active audio should yield non-zero peaks");

        let mini = vis.get_mini_bars(8, true);
        assert!(!mini.trim().is_empty(), "Mini bars should show activity when playing");
    ...
    fn test_mini_visualizer_independence_and_block_levels() {
        ...
        for c in mini.chars() {
            assert!(BLOCK_CHARS.contains(&c), "Character '{c}' should be in BLOCK_CHARS");
        }
